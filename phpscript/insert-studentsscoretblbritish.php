...
if ($countGetstudent_session > 0) {
    do {
        ...
        $sqlGetscore = "SELECT * FROM `britishresult` WHERE StudentID='$studentid' AND ClassID = '$classid' AND Term = '$term' AND SectionID = '$classsectionactual' AND SubjectID = '$subjects' AND Session = '$session' AND Term = '$term'";
        ...
        if ($countGetscore > 0) {
            ...
            $sqlInsert = ("INSERT INTO `britishresult`(`StudentID`, `ClassID`, `SectionID`, `SubjectID`, `Session`, `Term`) 
