...
try {
    ...
    foreach (array('consolidated block' => $consolidated136) as $name => $sql) {
    ...
    $rejected = false;
    try {
        migration136RunSql($pdo, $consolidated136);
    } catch (PDOException $exception) {
        $rejected = true;
    }
    migration136Assert($rejected, 'Consolidated SQL must reject a database without the Question Bank.');
    ...
    echo "Migration 136 database tests passed: consolidated SQL reruns, CodeIgniter up/down, preserved records, creation without level, and wrong-schema preflight." . PHP_EOL;
} finally {
