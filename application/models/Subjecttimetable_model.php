...
class Subjecttimetable_model extends MY_Model {
    ...
    public function add2($delete_array, $insert_array, $update_array) {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        if (!empty($delete_array)) {
        ...
        if (isset($update_array) && !empty($update_array)) {
            ...
            $this->db->update_batch('subjecttables', $update_array, 'id');
        }

        if (isset($insert_array) && !empty($insert_array)) {

            $this->db->insert_batch('subjecttables', $insert_array);
            $count = count($insert_array);
            $id = $this->db->insert_id();
            $loop = $id - $count;
            for ($x = $id; $x > $loop; $x--) {
    ...
    public function getBySubject2GroupDayClassSection($subject_group_id, $day, $class_id, $section_id) {

        $this->db->select('subjecttables.*');
        $this->db->from('subjecttables');
        $this->db->join('subject_group_subjects', 'subjecttables.subject_group_subject_id = subject_group_subjects.id');
        $this->db->join('staff', 'staff.id = subjecttables.staff_id');
        $this->db->where('subjecttables.class_id', $class_id);
        $this->db->where('subjecttables.section_id', $section_id);
        $this->db->where('subjecttables.day', $day);
        $this->db->where('subjecttables.subject_group_id', $subject_group_id);
        $this->db->where('staff.is_active', 1);
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function getSubject2ByClassandSectionDay($class_id, $section_id, $day) {
        $subject_condition = "";
        $userdata = $this->customlib->getUserData();
            ...
            if ($userdata["class_teacher"] == 'yes') {
                ...
                if (!empty($my_classes)) {
                    if (in_array($class_id, $my_classes)) {
                        ...
                        $my_subjects = $this->teacher_model->get_subjectby_classid2($class_id, $section_id, $userdata['id']);
        ...
        $sql = "SELECT `subject_group_subjects`.`subject_id`,subjects.name as `subject_name`,subjects.code,subjects.type,staff.name,staff.surname,staff.employee_id,`subjecttables`.* FROM `subjecttables` JOIN `subject_group_subjects` ON `subjecttables`.`subject_group_subject_id` = `subject_group_subjects`.`id`inner JOIN subjects on subject_group_subjects.subject_id = subjects.id INNER JOIN staff on staff.id=subjecttables.staff_id   WHERE `subjecttables`.`class_id` = " . $class_id . " AND `subjecttables`.`section_id` = " . $section_id . " AND `subjecttables`.`day` = " . $this->db->escape($day) . " AND `subjecttables`.`session_id` = " . $this->current_session . "" . $subject_condition;
    ...
    public function getByStaffandDay2($staff_id, $day_value) {

        $sql = "SELECT `classes`.`class`,`sections`.`section`,`subject_group_subjects`.`subject_id`,`sub`.`name` as `subject_name`,`sub`.`code` as `subject_code`,`subjecttables`.* FROM `subjecttables` INNER JOIN `classes` on classes.id = `subjecttables`.`class_id` INNER JOIN sections on `sections`.`id`=`subjecttables`.`section_id` INNER JOIN `subject_group_subjects` on `subject_group_subjects`.`id`=`subjecttables`.`subject_group_subject_id` INNER JOIN `subjects` as `sub` on `sub`.`id`=`subject_group_subjects`.`subject_id`  WHERE subjecttables.staff_id=" . $this->db->escape($staff_id) . " and subjecttables.session_id =" . $this->current_session . " and subjecttables.day=" . $this->db->escape($day_value);
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
