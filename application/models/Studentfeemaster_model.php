...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function getStudentFees($student_session_id, $term='all')
    {	
		$term = $term;
		if ($term == 'all') {
			...
			on fee_groups.id=fee_session_groups.fee_groups_id  
			...
			INNER JOIN fee_groups 
			on fee_groups.id=fee_session_groups.fee_groups_id  
			WHERE `student_session_id` = '$student_session_id' AND `term` =  '$term' 
			ORDER BY `student_fees_master`.`id`";
		}
