...
class Setting_model extends MY_Model {
    ...
    public function get($id = null) {
        ...
        $this->db->select('sch_settings.id,term,sch_settings.lang_id,sch_settings.languages,sch_settings.class_teacher,sch_settings.is_rtl,sch_settings.cron_secret_key, sch_settings.timezone,
    ...
    public function getSchoolDetail($id = null) {
        ...
        $this->db->select('sch_settings.id,sch_settings.lang_id,sch_settings.is_rtl,sch_settings.timezone,
          ...
          sch_settings.address,sch_settings.dise_code,sch_settings.date_format,sch_settings.currency,sch_settings.currency_symbol,sch_settings.start_month,sch_settings.start_week,sch_settings.session_id,sch_settings.image,sch_settings.theme,sessions.session,sch_settings.whatsapp_support_enabled,sch_settings.whatsapp_support_number,sch_settings.whatsapp_support_message'
        );
        $this->db->from('sch_settings');
        $this->db->join('sessions', 'sessions.id = sch_settings.session_id');
        $this->db->join('languages', 'languages.id = sch_settings.lang_id');
    ...
    public function getSetting() {
        ...
        $this->db->select('sch_settings.id,term,sch_settings.attendence_type,sch_settings.lang_id,sch_settings.is_rtl,sch_settings.fee_due_days,sch_settings.class_teacher,sch_settings.cron_secret_key,sch_settings.timezone,
    ...
    public function add($data) {
        ...
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $posted_term = $this->input->post('term');
        if (!array_key_exists('term', $data) && $posted_term !== null && $posted_term !== '') {
            $data['term'] = $posted_term;
        }
        if (isset($data['id'])) {
	...
	public function getCurrentTerm() {
        $term_result = $this->get();

        return $term_result[0]['term'];
    }
