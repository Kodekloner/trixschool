...
$gatewayAgentList = $pageItems($gateway_agents);
$gatewayCommandList = $pageItems($gateway_commands);
    ...
    <section class="content">
        ...
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs" role="tablist">
                ...
                <li><a href="#bio-connector" data-toggle="tab"><i class="fa fa-cloud-upload"></i> School Computer Connector</a></li>
            ...
            <div class="tab-content">
                    ...
                    <div class="row">
                        <div class="col-md-6"><div class="box box-info"><div class="box-header with-border"><h3 class="box-title">System health</h3></div><div class="box-body">
                            <dl class="dl-horizontal">
                                <?php $overviewGatewayState = $gatewayConnectionState($latestGatewayAgent); ?>
                                <dt>Connector</dt><dd><span class="label label-<?php echo $overviewGatewayState[1]; ?>"><?php echo html_escape($overviewGatewayState[0]); ?></span></dd>
                                <dt>Last heartbeat</dt><dd><?php echo html_escape($latestGatewayAgent && !empty($latestGatewayAgent['last_heartbeat_at']) ? $latestGatewayAgent['last_heartbeat_at'] : 'Never'); ?></dd>
                                <dt>Last event batch</dt><dd><?php echo html_escape(isset($dashboard['last_gateway_seen']) && $dashboard['last_gateway_seen'] ? $dashboard['last_gateway_seen'] : 'Never'); ?></dd>
                ...
                <div class="tab-pane" id="bio-setup">
                    <div class="box box-default"><div class="box-header with-border"><h3 class="box-title">Go-live checklist</h3></div><div class="box-body"><p>SchoolLift blocks Live mode until every safety check below passes. Run the physical terminal in Shadow mode before enabling official writes.</p><ul class="bio-checklist"><?php foreach (array('integration' => 'Exactly one active ZKBio integration for this connector path', 'physical_device' => 'Enabled physical bidirectional terminal on that integration', 'punch_states' => 'Distinct IN and OUT punch states on that integration', 'identity_mapping' => 'At least one active identity mapping', 'connector_online' => 'Exactly one Windows connector heartbeat received within ten minutes', 'connector_healthy' => 'That connector can reach ZKBio and synchronized successfully', 'connector_queue_clear' => 'That connector has no waiting, retrying, or permanently failed item', 'shadow_event' => 'Every enabled physical terminal produced an accepted Shadow event', 'exceptions_clear' => 'No open biometric exceptions') as $check => $label) { $ok = !empty($liveReadiness['checks'][$check]); ?><li><i class="fa <?php echo $ok ? 'fa-check-circle' : 'fa-times-circle'; ?>"></i> <?php echo html_escape($label); ?></li><?php } ?></ul><?php if (!empty($liveReadiness['ready'])) { ?><div class="alert alert-success">Technical preflight passed. Complete the recommended five-day Shadow pilot and confirm your password before Live.</div><?php } else { ?><div class="alert alert-warning">Live is locked until this checklist is complete.</div><?php } ?></div></div>
                ...
                <div class="tab-pane" id="bio-connector">
                    <div class="bio-connector-summary">
                        <strong>No routine command line is required.</strong> The connector on the school Windows computer contacts SchoolLift automatically once every minute in both Shadow and Live. A button below creates a fixed, audited request; it never gives the website a command prompt or access to arbitrary programs on that computer.
                    </div>
                    <?php if (!$gatewayAgentList) { ?><div class="alert alert-warning"><strong>No school computer has reported yet.</strong> Create an integration token under Setup &amp; Devices, complete the one-time Windows Gateway Setup/Manager on the ZKBio computer, and wait up to one minute. Website actions cannot wake an uninstalled, stopped, or switched-off computer.</div><?php } ?>

                    <div class="row">
                        <?php foreach ($gatewayAgentList as $agent) {
                            $agentState = $gatewayConnectionState($agent);
                            $providerState = $agent['provider_reachable'] === null ? 'Not checked' : (!empty($agent['provider_reachable']) ? 'Connected' : 'Needs attention');
                            $syncState = $agent['last_sync_ok'] === null ? 'Not run' : (!empty($agent['last_sync_ok']) ? 'Successful' : 'Failed');
                        ?><div class="col-md-6"><div class="box box-<?php echo $agentState[1] === 'danger' ? 'danger' : ($agentState[1] === 'warning' ? 'warning' : 'success'); ?>"><div class="box-header with-border"><h3 class="box-title"><i class="fa fa-windows"></i> <?php echo html_escape($agent['gateway_id']); ?></h3><span class="label label-<?php echo $agentState[1]; ?> pull-right"><?php echo html_escape($agentState[0]); ?></span></div><div class="box-body">
                            <dl class="dl-horizontal">
                                <dt>Integration</dt><dd><?php echo html_escape(isset($integrationNames[(int) $agent['integration_id']]) ? $integrationNames[(int) $agent['integration_id']] : '#' . (int) $agent['integration_id']); ?></dd>
                                <dt>Version</dt><dd><?php echo html_escape($agent['gateway_version']); ?></dd>
                                <dt>Last heartbeat</dt><dd><?php echo html_escape($agent['last_heartbeat_at']); ?> UTC</dd>
                                <dt>ZKBio connection</dt><dd><?php echo html_escape($providerState); ?></dd>
                                <dt>Last sync</dt><dd><?php echo html_escape($syncState); ?><?php echo !empty($agent['last_sync_at']) ? ' — ' . html_escape($agent['last_sync_at']) . ' UTC' : ''; ?></dd>
                                <dt>Waiting/retry/failed</dt><dd><?php echo (int) $agent['queue_pending']; ?> / <?php echo (int) $agent['queue_retry']; ?> / <strong class="<?php echo (int) $agent['queue_dead'] > 0 ? 'text-red' : ''; ?>"><?php echo (int) $agent['queue_dead']; ?></strong></dd>
                                <dt>Provider cursor</dt><dd class="bio-code"><?php echo html_escape($agent['provider_cursor'] ?: 'None'); ?></dd>
                            </dl>
                            <?php if (!empty($agent['last_error'])) { ?><div class="alert alert-danger"><strong>Safe error summary:</strong> <?php echo html_escape($agent['last_error']); ?></div><?php } ?>
                        </div></div></div><?php } ?>
                    </div>

                    <div class="box box-primary"><div class="box-header with-border"><h3 class="box-title">Safe website actions</h3></div><div class="box-body">
                        <p>Choose the integration used by this school computer. The next automatic heartbeat normally collects the request within one minute. Results then appear in the history below.</p>
                        <?php if ($canEditBiometric && $integrationList) { ?>
                            <div class="bio-connector-actions">
                                <form method="post" action="<?php echo site_url('admin/biometricattendance/gatewaycommand'); ?>"><?php echo $csrfField; ?><input type="hidden" name="command_type" value="connection_test"><select name="integration_id" required class="form-control input-sm"><option value="">Choose integration</option><?php foreach ($integrationList as $integration) { ?><option value="<?php echo (int) $integration['id']; ?>" <?php echo empty($integration['is_active']) ? 'disabled' : ''; ?>><?php echo html_escape($integration['name']); ?></option><?php } ?></select><button class="btn btn-info btn-sm" <?php echo !in_array($mode, array('shadow', 'live'), true) ? 'disabled' : ''; ?>><i class="fa fa-stethoscope"></i> Check connections</button></form>
                                <form method="post" action="<?php echo site_url('admin/biometricattendance/gatewaycommand'); ?>"><?php echo $csrfField; ?><input type="hidden" name="command_type" value="sync_now"><select name="integration_id" required class="form-control input-sm"><option value="">Choose integration</option><?php foreach ($integrationList as $integration) { ?><option value="<?php echo (int) $integration['id']; ?>" <?php echo empty($integration['is_active']) ? 'disabled' : ''; ?>><?php echo html_escape($integration['name']); ?></option><?php } ?></select><button class="btn btn-primary btn-sm" <?php echo !in_array($mode, array('shadow', 'live'), true) ? 'disabled' : ''; ?>><i class="fa fa-refresh"></i> Synchronize now</button></form>
                            </div>
                            <?php if (!in_array($mode, array('shadow', 'live'), true)) { ?><p class="help-block">Switch to Shadow mode before using the physical school-computer connector.</p><?php } ?>
                            <hr>
                            <form method="post" action="<?php echo site_url('admin/biometricattendance/gatewaycommand'); ?>" class="form-inline"><?php echo $csrfField; ?><input type="hidden" name="command_type" value="retry_failed"><select name="integration_id" required class="form-control input-sm"><option value="">Choose integration</option><?php foreach ($integrationList as $integration) { ?><option value="<?php echo (int) $integration['id']; ?>" <?php echo empty($integration['is_active']) ? 'disabled' : ''; ?>><?php echo html_escape($integration['name']); ?></option><?php } ?></select> <input name="confirmation" required autocomplete="off" class="form-control input-sm bio-code" placeholder="RETRY_FAILED_ITEMS"> <button class="btn btn-danger btn-sm" <?php echo !in_array($mode, array('shadow', 'live'), true) ? 'disabled' : ''; ?> onclick="return confirm('Retry permanently failed items only after correcting their cause?');"><i class="fa fa-repeat"></i> Retry failed items</button><p class="help-block">Use this only after correcting the displayed cause. It never accepts arbitrary command text.</p></form>
                        <?php } elseif (!$integrationList) { ?><p>Create an active ZKBio integration before requesting connector actions.</p><?php } else { ?><p>Your role can view connector status but cannot request actions.</p><?php } ?>
                    </div></div>

                    <div class="box box-default"><div class="box-header with-border"><h3 class="box-title">Recent connector actions</h3></div><div class="box-body bio-table-wrap"><table class="table table-condensed table-striped"><thead><tr><th>Requested</th><th>Integration</th><th>Action</th><th>Status</th><th>Collected by</th><th>Completed</th><th>Safe result</th></tr></thead><tbody><?php foreach ($gatewayCommandList as $command) { $commandResult = !empty($command['result_json']) ? json_decode($command['result_json'], true) : null; ?><tr><td><?php echo html_escape($command['queued_at']); ?></td><td><?php echo html_escape(isset($integrationNames[(int) $command['integration_id']]) ? $integrationNames[(int) $command['integration_id']] : '#' . (int) $command['integration_id']); ?></td><td><?php echo html_escape(str_replace('_', ' ', $command['command_type'])); ?></td><td><span class="label label-<?php echo $command['status'] === 'succeeded' ? 'success' : ($command['status'] === 'failed' ? 'danger' : ($command['status'] === 'expired' ? 'default' : 'warning')); ?>"><?php echo html_escape($command['status']); ?></span></td><td class="bio-code"><?php echo html_escape($command['gateway_id'] ?: 'Waiting'); ?></td><td><?php echo html_escape($command['completed_at']); ?></td><td><?php if (is_array($commandResult)) { ?><details><summary>View</summary><pre><?php echo html_escape(json_encode($commandResult, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)); ?></pre></details><?php } else { echo '—'; } ?></td></tr><?php } ?><?php if (!$gatewayCommandList) { ?><tr><td colspan="7" class="bio-empty">No website connector actions have been requested.</td></tr><?php } ?></tbody></table></div></div>
                </div>
