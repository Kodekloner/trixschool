...
<div class="content-wrapper">
    <section class="content-header">
        <h1><i class="fa fa-pencil-square-o"></i> Manual Marking <small><?php echo html_escape($exam->exam); ?></small></h1>
    </section>
    <section class="content">
        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                ...
                <p class="help-block">Draft marks remain incomplete. Finalized marks are included in calculation. Finalizing the attempt automatically posts to its configured destination but does not publish the report card.</p>
            </div>
        </div>
        ...
        <?php foreach ($detail['papers'] as $paper_row) {
            $paper = $paper_row['paper'];
            $summary = $paper_row['attempt_paper'];
            $latest = empty($paper_row['history']) ? array() : $paper_row['history'][0]; ?>
            <div class="box box-warning">
                <div class="box-header with-border"><h3 class="box-title">Paper-delivered: <?php echo html_escape($paper['title']); ?></h3></div>
                <div class="box-body">
                    <p>Enter the total raw mark from the physical answer script. Maximum: <strong><?php echo number_format((float) $paper['raw_max_score'], 2); ?></strong>; contribution: <?php echo number_format((float) $paper['contribution_score'], 2); ?>.</p>
                    <form method="post" action="<?php echo base_url('admin/onlineexam/operationPaperMark/' . $exam->id); ?>">
                        <?php echo $this->customlib->getCSRF(); ?>
                        <input type="hidden" name="onlineexam_workflow_token" value="<?php echo html_escape($workflow_csrf); ?>">
                        <input type="hidden" name="attempt_id" value="<?php echo (int) $attempt['id']; ?>">
                        <input type="hidden" name="paper_id" value="<?php echo (int) $paper['id']; ?>">
                        <div class="row">
                            <div class="form-group col-md-2"><label>Raw mark</label><input class="form-control" type="number" step="0.01" min="0" max="<?php echo html_escape($paper['raw_max_score']); ?>" required name="raw_marks" value="<?php echo html_escape(isset($summary['manual_score']) ? $summary['manual_score'] : ''); ?>"></div>
                            ...
                            <div class="form-group col-md-7"><label>Marker's note</label><input class="form-control" maxlength="5000" name="remark" value="<?php echo html_escape(isset($latest['remark']) ? $latest['remark'] : ''); ?>"></div>
                        </div>
                        <div class="form-group"><label>Rubric evidence (optional JSON)</label><textarea class="form-control" name="rubric_json" rows="2" placeholder='{"criterion":"mark awarded"}'><?php echo html_escape(isset($latest['rubric_json']) ? $latest['rubric_json'] : ''); ?></textarea></div>
                        <button class="btn btn-warning" type="submit">Save paper mark</button>
                        <?php if (!empty($paper_row['history'])) { ?><span class="text-muted">Version <?php echo (int) $latest['marking_version']; ?>; <?php echo html_escape($latest['status']); ?></span><?php } ?>
                    </form>
                </div>
            </div>
        <?php } ?>
        ...
        <?php foreach ($detail['answers'] as $index => $answer) {
            ...
            <div class="box box-default">
                <div class="box-header with-border">
                    <h3 class="box-title"><?php echo html_escape($answer['paper_title']); ?> — Manual question <?php echo $index + 1; ?> <small><?php echo html_escape($answer['question_type']); ?></small></h3>
                    <span class="pull-right label label-<?php echo $answer['marking_status'] === 'finalized' ? 'success' : 'warning'; ?>"><?php echo html_escape($answer['marking_status']); ?></span>
                </div>
                <div class="box-body">
                    ...
                    <form method="post" action="<?php echo base_url('admin/onlineexam/operationManualMark/' . $exam->id); ?>">
                        ...
                        <button class="btn btn-primary" type="submit">Save manual mark</button>
        ...
        <?php if (empty($detail['answers']) && empty($detail['papers'])) { ?>
            <div class="alert alert-info">There are no answered manual questions or paper-delivered scores to enter. Objective-only attempts are finalized automatically on submission.</div>
        <?php } ?>

        <div class="box box-success">
            <div class="box-body">
                <form method="post" action="<?php echo base_url('admin/onlineexam/operationFinalize/' . $exam->id); ?>" onsubmit="return confirm('Finalize all marks and automatically synchronize this result?');">
                    <?php echo $this->customlib->getCSRF(); ?>
                    <input type="hidden" name="onlineexam_workflow_token" value="<?php echo html_escape($workflow_csrf); ?>">
                    <input type="hidden" name="attempt_id" value="<?php echo (int) $attempt['id']; ?>">
                    <button class="btn btn-success" type="submit"><i class="fa fa-check"></i> Finalize marking and synchronize result</button>
                    <span class="help-block">If an unrelated manual score already occupies the destination, synchronization records a conflict and does not overwrite it.</span>
                </form>
            </div>
        </div>
    </section>
