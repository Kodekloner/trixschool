...
class Subjecttable extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('admin/subjecttable/timetableList', $data);
    ...
    public function mytimetable()
    {
        ...
        foreach ($days as $day_key => $day_value) {
            $data['timetable'][$day_value] = $this->subjecttable_model->getByStaffandDay($staff_id, $day_key);
        }

        $this->load->view('layout/header', $data);
        if ($is_admin) {
            $this->load->view('admin/subjecttable/admintimetable', $data);
        } else {
            $this->load->view('admin/subjecttable/mytimetable', $data);
        }
        $this->load->view('layout/footer', $data);
    }
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/subjecttable/timetableCreate', $data);
            $this->load->view('layout/footer', $data);
        } else {
     ...
     public function classreport()
    {
        ...
        if ($this->form_validation->run() == true) {
            if (isset($_POST['search'])) {
                ...
                foreach ($days as $day_key => $day_value) {
                    $class_id              = $this->input->post('class_id');
                    $section_id            = $this->input->post('section_id');
                    $days_record[$day_key] = $this->subjecttimetable_model->getSubject2ByClassandSectionDay($class_id, $section_id, $day_key);
                }
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/subjecttable/classreport', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/subjecttable/timetableEdit', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $data = array(
                'id'   => $id,
    ...
    public function getBydategroupclasssection()
    {
        ...
        $prev_record = $this->subjecttimetable_model->getBySubject2GroupDayClassSection($subject_group_id, $day, $class_id, $section_id);
        ...
        $data['html'] = $this->load->view('admin/subjecttable/addrow', $data, true);
    ...
    public function savegroup()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $result       = $this->subjecttimetable_model->add2($delete_array, $insert_array, $update_array);
            if ($result) {
    ...
    public function getteachertimetable()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            foreach ($days as $day_key => $day_value) {
                $data['timetable'][$day_value] = $this->subjecttimetable_model->getByStaffandDay2($staff_id, $day_key);
            }

            $timetable_page = $this->load->view('admin/subjecttable/_partialgetteachertimetable', $data, true);
