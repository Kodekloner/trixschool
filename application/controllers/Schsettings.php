...
class Schsettings extends Admin_Controller
{
	...
	private function storeSettingImage($id, $field, $folder)
	{
		if (!isset($_FILES["file"]) || empty($_FILES['file']['name'])) {
			$array = array('success' => false, 'error' => '', 'message' => $this->lang->line('logo_file_is_required'));
		...
		if ($extension === '') {
			$array = array('success' => false, 'error' => '', 'message' => $this->lang->line('file_type_not_allowed'));
			echo json_encode($array);
			return;
		}

		$upload_result = upload_to_s3($_FILES["file"]["tmp_name"], $fileInfo, $extension, $folder);

		if (empty($upload_result['success'])) {
			$array = array(
				'success' => false,
				'error'   => isset($upload_result['error']) ? $upload_result['error'] : '',
				'message' => isset($upload_result['error']) ? $upload_result['error'] : 'Upload failed',
			);
			echo json_encode($array);
			return;
		}

		$data_record = array('id' => $id, $field => $upload_result['s3_key']);
	...
	public function ajax_editlogo()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->storeSettingImage($id, 'image', 'uploads/school_content/logo/');
		}
	}
	...
	public function ajax_editadmin_smalllogo()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$id = $this->input->post('id');
			$this->storeSettingImage($id, 'admin_small_logo', 'uploads/school_content/admin_small_logo/');
		}
	}
	...
	public function ajax_editadmin_adminlogo()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$id = $this->input->post('id');
			$this->storeSettingImage($id, 'admin_logo', 'uploads/school_content/admin_logo/');
		}
	}
	...
	public function ajax_schedit()
	{
		...
		$this->form_validation->set_rules('term', 'term', 'trim|required|xss_clean');
	...
	public function ajax_applogo()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$id = $this->input->post('id');
			$this->storeSettingImage($id, 'app_logo', 'uploads/school_content/logo/app_logo/');
		}
	}
